...
import type { Plan as PlanType } from "@/components/PlanFormDialog";
import type { Category } from "@/components/CategoryManager";

import type { Category as CategoryType } from "@/components/CategoryManager";
        ...
        <div className="container mx-auto px-6">
          ...
          >
            ...
            <h1 className="text-4xl md:text-5xl font-bold font-display mb-4">
              <span className="text-gradient">Pilihan Paket</span> Harga
            </h1>
            <p className="text-muted-foreground text-lg max-w-2xl mx-auto">
              Pilih paket yang paling sesuai untuk memulai, mengembangkan, dan mengelola karya musik Anda.
            </p>
          </motion.div>
          ...
          {loading ? (
            ...
            categories.map(category => (
              <section key={category.id} className="mb-24">
                <Link to={`/pricing/${category.slug}`} className="group">
                  <div className="flex items-center gap-3 mb-10 justify-center text-center">
                      {getIconForCategory(category.name)}
                      <h2 className="text-2xl md:text-3xl font-bold font-display group-hover:text-primary transition-colors">{category.name}</h2>
                  </div>
                </Link>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
                  {plans
